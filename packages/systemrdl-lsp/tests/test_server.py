...
def test_compile_text_returns_root_on_valid_buffer(tmp_path):
    ...
    messages, roots, tmp_file, _consumed, _node_index = _compile_text(uri, VALID_RDL)
    try:
...
def test_compile_text_returns_no_root_on_parse_error(tmp_path):
    ...
    uri = (tmp_path / "x.rdl").as_uri()
    messages, roots, tmp_file, _consumed, _node_index = _compile_text(uri, INVALID_RDL)
    try:
        assert roots == []
        errors = [m for m in messages if m.severity in (Severity.ERROR, Severity.FATAL)]
...
def test_compile_text_translates_temp_path_to_original_uri(tmp_path):
    ...
    uri = original.as_uri()
    messages, _roots, tmp_file, _consumed, _node_index = _compile_text(uri, INVALID_RDL)
    try:
...
def test_definition_resolves_top_level_type(tmp_path):
    ...
    uri = (tmp_path / "x.rdl").as_uri()
    _msgs, roots, tmp, _consumed, _node_index = _compile_text(uri, TYPED_RDL)
    try:
...
def test_rename_locations_covers_def_and_uses(tmp_path):
    ...
    _msgs, roots, tmp, _consumed, _node_index = _compile_text(rdl_path.as_uri(), TYPED_RDL)
...
def test_rename_skips_unrelated_types(tmp_path):
    ...
    _msgs, roots, tmp, _consumed, _node_index = _compile_text(rdl_path.as_uri(), src)
...
def test_multi_addrmap_elaborates_each_top_level_definition(tmp_path):
    ...
    """
    uri = (tmp_path / "x.rdl").as_uri()
    _msgs, roots, tmp, _consumed, _node_index = _compile_text(uri, MULTI_ADDRMAP_RDL)
    try:
...
def test_cache_unlinks_old_temp_file_on_replace(tmp_path):
    ...
    _msgs1, root1, tmp1, _consumed, _node_index = _compile_text(uri, VALID_RDL)
    assert tmp1.exists()
    ...
    _msgs2, root2, tmp2, _consumed, _node_index = _compile_text(uri, VALID_RDL)
...
def test_cache_clear_unlinks_all_temps(tmp_path):
    ...
    for i in range(3):
        ...
        _msgs, root, tmp, _consumed, _node_index = _compile_text(uri, VALID_RDL)
...
def test_hover_on_field_includes_access_and_reset(tmp_path):
    uri = (tmp_path / "x.rdl").as_uri()
    _msgs, root, tmp, _consumed, _node_index = _compile_text(uri, SAMPLE_RDL)
    try:
...
def test_timeout_path_preserves_last_good_cache(tmp_path):
    ...
    """
    uri = (tmp_path / "x.rdl").as_uri()
    msgs, root, tmp, _consumed, _node_index = _compile_text(uri, VALID_RDL)
...
def test_bridge_keyword_in_completion_and_hover(tmp_path):
    ...
    _msgs, roots, tmp, _consumed, _node_index = _compile_text(rdl_path.as_uri(), rdl)
    try:
...
def test_cross_file_diagnostics_bucket_by_source(tmp_path):
    ...
    msgs, _roots, tmp, _consumed, _node_index = _compile_text(master.as_uri(), master.read_text())
    try:
...
def test_compile_text_accepts_perl_safe_opcodes(tmp_path):
    ...
    """
    uri = (tmp_path / "x.rdl").as_uri()
    msgs, roots, tmp, _consumed, _node_index = _compile_text(
        uri,
        VALID_RDL,
