...
def _roots(rdl_text: str, tmp_path: Path, name: str = "x.rdl"):
    ...
    msgs, roots, tmp, consumed, _node_index = _compile_text(p.as_uri(), rdl_text)
...
def test_consumed_files_includes_includee(tmp_path):
    ...
    msgs, roots, tmp, consumed, _node_index = _compile_text(main.as_uri(), main_text)
    try:
...
def test_consumed_files_excludes_temp_path(tmp_path):
    ...
    msgs, roots, tmp, consumed, _node_index = _compile_text(main.as_uri(), SIMPLE_RDL)
    try:
        assert roots
