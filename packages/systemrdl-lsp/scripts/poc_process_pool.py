...
def probe_pickle_rootnode() -> dict:
    ...
    _msgs, roots, tmp_path, _consumed, _node_index = _compile_text(rdl_path.as_uri(), text)
...
def probe_compression() -> dict:
    ...
    _msgs, roots, tmp_path, _, _node_index = _compile_text(big.as_uri(), big.read_text())
    if not roots:
...
def _serialize_in_subprocess(uri: str, text: str) -> dict:
    ...
    msgs, roots, tmp_path, consumed, _node_index = _compile_text(uri, text)
    spine_envelopes = []
    full_envelopes = []
    for r in roots:
        try:
